...
export default () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const uriProfile =
        (await AsyncStorage.getItem("uriProfile")) ||
        require("../../assets/imgs/genericProfile.png");
      setState({ ...state, email, nome, uri: uriProfile });
    };
    getData();
  }, []);
        ...
        <BottomModal
          ...
          content={
            <TextInput
              ...
              onBlur={async () => AsyncStorage.setItem("nome", state.nome)}
            />
          }
        />
      </KeyboardAvoidingView>
